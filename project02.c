...
// entry in linked list, contains passwords, sha256 hash and pointer to next
// node
struct entry {
...
// hasing function given by professor
...
// Transformation function (leet char)
...
void add_one(char *password) {
  strcat(password, "1"); // strcat connects two strings
}

// Function to insert a new entry into the sorted linked list
// insters new node and sorts it in ascending order.
void insertSorted(struct entry **head, const char *password) {
  ...
  if (newEntry == NULL) {
    perror(
        "Error allocating memory"); // perror() is used to print error message
    exit(1);
  }
...
void writeDictionaryToFile(const char *filename, struct entry *head) {
  FILE *file = fopen(filename, "w"); // file opens in writing mode
  if (file == NULL) {
...
int main(int argc, char *argv[]) {
  ...
  FILE *passwordFile = fopen(argv[1], "r"); // file opens in reading mode only
