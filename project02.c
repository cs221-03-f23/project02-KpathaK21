...
void insertSorted(struct entry** head, const char* password) {
 ...
 // Entry does not exist, insert it
     ...
     // copy the password into the input array so that it can be hashed till the null character does not appear
 ...
 strcpy(newEntry->hash, hash);
     ...
     if (*head == NULL || strcmp((*head)->hash, newEntry->hash) > 0) {
         ...
         *head = newEntry;
         ...
         struct entry* current = *head;
         while (current->next != NULL && strcmp(current->next->hash, newEntry->hash) <= 0) {
             current = current->next;
         }
